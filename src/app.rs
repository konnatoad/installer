                    ...
                    if let Some(action) = welcome::show(
                        ...
                        match action {
                            ...
                            welcome::WelcomeAction::DownloadInstaller => {
                                ...
                                std::thread::spawn(move || {
                                    *dl_ref.lock().unwrap() =
                                        welcome::InstallerDlState::Downloading;
                                    ctx.request_repaint();
                                    match crate::install::download_installer_to_downloads() {
                                        Ok(path) => {
                                            *dl_ref.lock().unwrap() =
                                                welcome::InstallerDlState::Done(path)
                                        }
                                        Err(e) => {
                                            *dl_ref.lock().unwrap() =
                                                welcome::InstallerDlState::Error(e.to_string())
                                        }
                                    }
                                    ctx.request_repaint();
                                });
                            ...
                            progress::ProgressAction::Continue => {
                                ...
                                let msg = if success {
                                    match state.operation {
                                        Operation::Install => {
                                            "Kadr was installed successfully!".to_owned()
                                        }
                                        Operation::Update => {
                                            "Kadr was updated successfully!".to_owned()
                                        }
                                        Operation::Uninstall => {
                                            "Kadr was uninstalled successfully.".to_owned()
                                        }
                                    }
                                } else {
                                    format!(
